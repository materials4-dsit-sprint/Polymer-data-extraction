...
def generate_system_prompt(
    ...
    return (
        ...
        "### COMPOSITION NAMING (CRITICAL - PROPER CHEMICAL NAMES):\n"
        "═══════════════════════════════════════════════════════════════════════════\n"
        "The 'composition' field MUST use the proper chemical name of the polymer, NOT informal labels.\n\n"
        "❌ NEVER use paper-internal labels as the composition:\n"
        "   - 'HBP 5', 'HBP 6', 'Polymer 1', 'P1a/2a', 'Monomer 1a', 'Monomer 2a'\n"
        "   - These are shorthand references, not chemical identities.\n\n"
        "✅ ALWAYS resolve to the actual chemical identity:\n"
        "   - Search the ENTIRE paper (abstract, introduction, synthesis, experimental, schemes, tables) "
        "for where each label is defined.\n"
        "   - Trace monomer/polymer definitions: e.g. 'HBP 6 from HBP 5' → find what HBP 5 is made from; "
        "'Monomer 4 = 1-phenethylindoline-2,3-dione' → use that chemical name.\n"
        "   - Use IUPAC name, common name, or structural description: poly(styrene), PMMA, "
        "hyperbranched polymer from 1-phenethylindoline-2,3-dione, polynaphthalene from iodobenzene and "
        "internal diyne 2a, etc.\n\n"
        "When monomers are defined elsewhere (e.g. 'Monomer 1a = iodobenzene', 'Monomer 2a = internal diyne'), "
        "substitute the chemical identity into the composition. If a polymer is a copolymer, list both "
        "monomer identities: e.g. 'poly(naphthalene) from 4-iodobenzene and 1,2-bis(4-methoxyphenyl)ethyne' "
        "rather than 'P1a/2a'.\n"
        "═══════════════════════════════════════════════════════════════════════════\n\n"
...
def generate_user_prompt(text: str) -> str:
    ...
    return (
        ...
        "For each polymer composition: search the full text (including synthesis, experimental, schemes) "
        "to resolve any labels (e.g. HBP 5, P1a/2a) to their proper chemical names before extracting. "
